...
- Cache ordering follows the authoritative registry activation timestamp. A stale distribution snapshot is rejected, while a newer authoritative activation may legitimately point to a lower bundle version or a different bundleId.
...
- `internal/authorization/cache_test.go`: bootstrap fail-closed, exact decision, LKG retention, stale activation rejection, authoritative activation replacement, org-scope fail-closed.
