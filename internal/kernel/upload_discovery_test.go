...
func TestHostFileToolIsOptInAndAdvertisesFileParameter(t *testing.T) {
	...
	if err!=nil {t.Fatal(err)}
	server:=httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter,r *http.Request){r.Header.Set("X-OUF-Gateway-Verified","true");h.ServeHTTP(w,r)}))
	defer server.Close()
	...
	for _,tool:=range list.Tools {
		if tool.Name=="source.file.upload" {
			...
			if !ok||len(files)!=1||files[0]!="file" {t.Fatalf("missing host fileParams: %#v",tool.Meta)}
			return
		}
	}
