...
func TestReferentialRetentionChainPurgesTerminalGraph(t *testing.T) {
	...
	var n int
	if err := s.Pool().QueryRow(ctx, `select ouf_mcp.purge_terminal_attempt_graph(transaction_timestamp()-interval '1 day',10)`).Scan(&n); err != nil { t.Fatal(err) }
	if n != 1 { t.Fatalf("purged=%d want=1", n) }
	...
	for name, query := range map[string]string{
		...
		var count int
		arg := any(attempt)
		if name == "window" { arg = window }
		if name == "group" { arg = group }
		if err := s.Pool().QueryRow(ctx, query, arg).Scan(&count); err != nil { t.Fatal(err) }
		if count != 0 { t.Fatalf("%s still present", name) }
	}
	var manifests int
	if err := s.Pool().QueryRow(ctx, `select count(*) from ouf_mcp.manifest_snapshot where manifest_checksum=$1`, checksum).Scan(&manifests); err != nil { t.Fatal(err) }
	if manifests != 1 { t.Fatal("manifest snapshot immutable retention boundary was violated") }
}
