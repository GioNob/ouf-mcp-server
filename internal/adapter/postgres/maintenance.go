...
func (s *Store) ReconcileDebtCaches(ctx context.Context, limit int) (DebtCacheReconciliationResult, error) {
	...
	for rows.Next() {
		var id uuid.UUID
		if err = rows.Scan(&id); err != nil { rows.Close(); return DebtCacheReconciliationResult{}, err }
		ids = append(ids, id)
	}
	rows.Close()
	if err = rows.Err(); err != nil { return DebtCacheReconciliationResult{}, err }
	...
	for _, id := range ids {
		...
		if beginErr != nil { return result, beginErr }
		...
		delta := int64(0)
		if err == nil { delta, err = debtCacheDelta(cached, authoritative) }
		if err == nil && delta != 0 {
		...
		if err != nil { _ = tx.Rollback(ctx); return result, err }
