...
func (s *Store) ReconcileDebtCaches(ctx context.Context, limit int) (DebtCacheReconciliationResult, error) {
	...
	for _, id := range ids {
		...
		if err == nil && delta != 0 {
			tag, e := tx.Exec(ctx, `update ouf_mcp.budget_window set current_unresolved_object_debt_total=$2 where budget_window_id=$1 and current_unresolved_object_debt_total=$3`, id, authoritative, cached)
			err = e
