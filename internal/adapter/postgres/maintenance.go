...
// DebtCacheReconciliationResult reports bounded repairs of the non-authoritative
// budget_window.current_unresolved_object_debt_total cache. budget_object_debt
...
// ReconcileDebtCaches checks at most limit windows. Every window is reconciled
// in its own short transaction and is locked before the authoritative ACTIVE
// debt sum is recomputed. This deliberately does not forgive debt by age/TTL.
func (s *Store) ReconcileDebtCaches(ctx context.Context, limit int) (DebtCacheReconciliationResult, error) {
		...
		select w.budget_window_id
		 ...
		 where w.current_unresolved_object_debt_total <> coalesce((
		       select sum(d.debt_amount)
		        ...
		        where d.budget_window_id=w.budget_window_id and d.debt_state='ACTIVE'
		 ),0)
		 order by w.window_start,w.budget_window_id
		 limit $1`, limit)
	if err != nil {
	...
	for _, id := range ids {
		...
		if err == nil && delta != 0 {
			...
			tag, err = tx.Exec(ctx, `update ouf_mcp.budget_window set current_unresolved_object_debt_total=$2 where budget_window_id=$1 and current_unresolved_object_debt_total=$3`, id, authoritative, cached)
