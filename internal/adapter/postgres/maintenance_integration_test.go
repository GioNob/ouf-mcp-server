...
func TestDebtCacheReconciliationPostgres(t *testing.T) {
	...
	if dsn == "" { t.Skip("MCP_TEST_DATABASE_URL is not set") }
	...
	if err != nil { t.Fatal(err) }
	result, err := store.ReconcileDebtCaches(ctx, 10); if err != nil { t.Fatal(err) }
	if result.Repaired < 1 { t.Fatalf("expected at least one repair: %+v", result) }
	var cached int64
	if err = store.Pool().QueryRow(ctx, `select current_unresolved_object_debt_total from ouf_mcp.budget_window where budget_window_id=$1`, window).Scan(&cached); err != nil { t.Fatal(err) }
	if cached != 4 { t.Fatalf("cached debt=%d want=4", cached) }
	var audits int
	if err = store.Pool().QueryRow(ctx, `select count(*) from ouf_mcp.audit_event where event_type='BUDGET_DEBT_CACHE_REPAIRED' and safe_detail->>'budgetWindowId'=$1`, window.String()).Scan(&audits); err != nil { t.Fatal(err) }
	if audits != 1 { t.Fatalf("repair audits=%d want=1", audits) }
}
