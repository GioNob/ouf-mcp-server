...
func TestManagedUploadStreamsExactBytesWithDelegationAndNoURL(t *testing.T) {
	...
	if err != nil || response.Status != 201 && response.Status != 200 || called != 1 {
		t.Fatalf("governed upload failed: status=%d calls=%d err=%v", response.Status, called, err)
	}
	in.Upload = nil
	if _, err := client.Execute(context.Background(), in, time.Second); err == nil || called != 1 {
		t.Fatal("upload without verified stream reached Gateway")
	}
}
