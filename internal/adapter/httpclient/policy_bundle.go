...
func (c *PolicyBundleClient) FetchActive(ctx context.Context) (authorization.ActivePolicyBundle, error) {
	...
	if len(raw) > maximum {
		return authorization.ActivePolicyBundle{}, errors.New("authorization envelope too large")
	}
	...
	if err := decoder.Decode(new(any)); err != io.EOF {
		return authorization.ActivePolicyBundle{}, errors.New("trailing authorization content")
	}
	var envelope struct {
	...
	if err := json.Compact(&compact, envelope.Bundle); err != nil {
		return authorization.ActivePolicyBundle{}, err
	}
	sum := sha256.Sum256(compact.Bytes())
	if out.ContentHash != hex.EncodeToString(sum[:]) {
		return authorization.ActivePolicyBundle{}, errors.New("authorization bundle hash mismatch")
	}
