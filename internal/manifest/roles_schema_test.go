...
func TestApplicationRoleInputsRemainClosedAndProposalOnly(t *testing.T) {
	...
	for _, c := range snapshot.Capabilities {
		if c.CapabilityID != "authorization.permissions.read" && c.CapabilityID != "authorization.permissions.propose" {
			continue
		}
		if c.CapabilityID == "authorization.permissions.propose" && c.MCPClass != "MCP_PROPOSAL_ONLY" {
			t.Fatal("role changes must remain proposals")
		}
		var s jsonschema.Schema
		if err := json.Unmarshal(c.InputSchema, &s); err != nil {
			t.Fatal(err)
		}
		...
		schemas[c.CapabilityID] = resolved
	}
	cases := []struct {
		cap, input string
		valid      bool
	...
	for _, tc := range cases {
		...
		s := schemas["authorization.permissions."+tc.cap]
		if s == nil {
