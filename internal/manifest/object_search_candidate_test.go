...
func TestR4aObjectSearchCandidateIsClosedAndNotAdvertised(t *testing.T) {
	...
	if candidate == nil { t.Fatal("missing object search candidate") }
	...
	for _, active := range snapshot.ToolEligible() {
		if active.CapabilityID == candidate.CapabilityID { t.Fatal("candidate was advertised") }
	}
	var schema struct {
	...
	if err := json.Unmarshal(candidate.InputSchema, &schema); err != nil { t.Fatal(err) }
	...
	for _, key := range []string{"type", "pageSize", "cursor"} {
		if _, ok := schema.Properties[key]; !ok { t.Fatalf("missing %s", key) }
	}
}
