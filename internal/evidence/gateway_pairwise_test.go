...
func (s *pairwiseEvidenceStore) InsertVerifiedEvidence(_ context.Context, in Verified) (Result, error) {
	...
	if old, ok := s.byKey[key]; ok {
		if old.PayloadHash != in.PayloadHash { return Result{}, ErrIngressConflict }
...
p=json.loads(sys.stdin.read()); MCPEvidenceIngressMediator(I()).ingest(json.dumps(p).encode(),{'X-Correlation-ID':'pairwise'},TrustedIdentity(sys.argv[1],'owner-worker','tenant','SERVICE','authn','authz',frozenset({'mcp.evidence.submit'})))`
	cmd := exec.Command("python3", "-c", script, service); cmd.Dir = gatewayRoot; cmd.Stdin = bytesReader(raw)
	out, err := cmd.CombinedOutput(); if err != nil { t.Fatalf("gateway ingress rejected payload: %v: %s", err, out) }
	var got map[string]any; if err = json.Unmarshal(out, &got); err != nil { t.Fatalf("invalid gateway canonical output: %s: %v", out, err) }
	return got
}

type reader []byte
func (r *reader) Read(p []byte) (int,error) { if len(*r)==0{return 0,os.ErrClosed}; n:=copy(p,*r); *r=(*r)[n:]; return n,nil }
func bytesReader(b []byte) *reader { r:=reader(b); return &r }

func TestGatewayEvidenceIngressPairwise(t *testing.T) {
	root := os.Getenv("GATEWAY_PAIRWISE_ROOT"); if root=="" { t.Skip("GATEWAY_PAIRWISE_ROOT is not set") }
	...
	for _, h := range canonical["ObjectHashes"].([]any) { input.ObjectHashes=append(input.ObjectHashes,h.(string)) }
	identity := TrustedIdentity{Authenticated:true,ServicePrincipalID:"ouf-api-gateway",BackendOwner:"udp-object-resolution"}
	first, err := svc.Ingest(context.Background(), identity, input); if err != nil || first.Replay { t.Fatalf("first ingress=%+v err=%v", first,err) }
	second, err := svc.Ingest(context.Background(), identity, input); if err != nil || !second.Replay || second.EvidenceRef!=first.EvidenceRef { t.Fatalf("replay=%+v err=%v",second,err) }
	input.ResultRef="result://udp/different"; if _,err=svc.Ingest(context.Background(),identity,input); err!=ErrIngressConflict { t.Fatalf("conflict err=%v",err) }
	if _,err=svc.Ingest(context.Background(),TrustedIdentity{Authenticated:true,ServicePrincipalID:"ouf-api-gateway",BackendOwner:"attacker"},input); err!=ErrOwnerMismatch { t.Fatalf("forged owner err=%v",err) }
	...
	if canonical["Kind"]!="OWNER_PROVES_NO_DISPATCH" || canonical["ActualDistinctObjects"].(float64)!=0 { t.Fatalf("no-dispatch not preserved: %+v",canonical) }
	...
	raw,_:=json.Marshal(bad); script:=`import json,sys
from tools.mcp_dispatch import BackendResponse,TrustedIdentity
from tools.mcp_evidence_ingress import MCPEvidenceIngressMediator
class I:
...
MCPEvidenceIngressMediator(I()).ingest(sys.stdin.buffer.read(),{'X-Correlation-ID':'pairwise'},TrustedIdentity('ouf-udp-object-resolution','w','t','SERVICE','a','z',frozenset({'mcp.evidence.submit'})))`
	cmd:=exec.Command("python3","-c",script); cmd.Dir=root; cmd.Stdin=bytesReader(raw); if err=cmd.Run(); err==nil { t.Fatal("unsupported hash version was accepted") }
	...
	var wg sync.WaitGroup; refs:=make(chan uuid.UUID,8); errs:=make(chan error,8)
	for i:=0;i<8;i++ { wg.Add(1); go func(){defer wg.Done(); r,e:=svc.Ingest(context.Background(),identity,concurrent); if e!=nil{errs<-e;return}; refs<-r.EvidenceRef}() }; wg.Wait(); close(refs); close(errs)
	for e:=range errs { t.Fatalf("concurrent ingress err=%v",e) }; var expected uuid.UUID; for ref:=range refs { if expected==uuid.Nil{expected=ref}; if ref!=expected{t.Fatalf("concurrent refs differ %s %s",expected,ref)} }
}
